...
import { CreateMemberRequest, Member, UpdateMemberRequest, UpdateMemberPasswordRequest } from './member.model';
...
export class MembersService {
  ...
  constructor() {
    // Another tab (or another screen sharing this cache key) invalidated our
    // data — refetch automatically so this list never goes stale silently.
    onInvalidate(this.httpCache, MEMBERS_CACHE_PATTERN, () => this.load());
  }
  ...
  /** New members always land at the end of the list — return to page 1 so the confirmation is visible without hunting for it. */
  ...
  async updatePassword(payload: UpdateMemberPasswordRequest): Promise<void> {
